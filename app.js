...
async function main() {
    const UUID = await getVariableValue('UUID', 'aaaaaaa6-bbbb-6ccc-accc-eeeeeeeeeee6'); // 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
    ...
    const DOMAIN = await getVariableValue('DOMAIN', 'ripper-don-281713011959.us-south1.run.app');// 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
